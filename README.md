...
[![Version](https://img.shields.io/badge/version-1.4.0%20%22Spruce%22-10b981.svg)](https://github.com)
...
### 🌲 4 Expressive Syntax Flavors in v1.4.0 ("Spruce"):

```html
<!-- Include via CDN -->
<script src="https://unpkg.com/pinejs-core@1.4.0/dist/pine.min.js" defer></script>
...
| Feature | Alpine.js (v3) | PineJS (v1.4 "Spruce") |
| :--- | :--- | :--- |
...
| **Bundle Size** | ~14 KB minified | **&lt; 9 KB minified**, zero dependencies |
...
| **Built-in Magics** | `$el`, `$refs`, `$watch`, `$dispatch`, `$nextTick`, `$root`, `$data`, `$id`, `$store` | All Alpine magics PLUS **`$signal`**, **`$persist`**, **`$history`**, **`$fetch`**, **`$intersect`**, **`$focus`** |
| **List & Conditionals** | `<template x-if>` / `<template x-for>` | Fine-grained **`p-if`**, **`p-for`**, `<template loop="...">`, `<template *="...">` |
| **SSR Hydration** | Not built-in | **`p-hydrate`** activation marker with signal re-attachment |
...
pinejs/
├── dist/
│   ├── pine.js             # Development bundle with full diagnostics (v1.4.0)
│   ├── pine.min.js         # Production bundle (< 9 KB)
│   ├── pine.esm.js         # ES Module export
│   └── pine.d.ts           # Official TypeScript declarations
├── docs/
│   ├── index.html          # Alpine.js-style interactive documentation site
│   ├── docs.css            # Clean typography & layout (Fira Code)
│   ├── docs.js             # Interactive docs search & navigation
...
| :--- | :--- | :--- | :--- | :--- |
| **`v1.4.0`** | **Spruce** | 2026-09-15 | **Latest Stable** | Configurable multi-prefix engine (`Pine.prefix`), prefix-free semantic HTML (`state`, `text`, `show`, `model`, `loop`), symbol shorthands (`🌲`, `⚡`, `~`, `?`, `*`), WAAPI timeline orchestrator (`Pine.timeline`), chameleon drop-in mode. |
| **`v1.3.0`** | **Cedar** | 2026-09-15 | Previous Stable | `p-animate` spring physics & keyframe animations, `Pine.devtools` runtime diagnostics bridge, getter receiver proxy binding. |
